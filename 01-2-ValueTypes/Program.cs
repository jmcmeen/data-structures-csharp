namespace _01_2_ValueTypes
{   
    /// <summary>
    /// Program demonstrating the use of value types
    /// </summary>
    internal static class Program
    {
        ...
        {
            //In C#, a value type is a type that represents a simple value, such as a number,
            //a character, or a Boolean value. Value types are stored in memory as a single unit,
            //and they are not reference types.

            //Here are some characteristics of value types in C#:

            //Stored in memory as a single unit: Value types are stored in memory as a single unit,
            //which means that they occupy a fixed amount of memory space.

            //Not reference types: Value types are not reference types, which means that they are not
            //stored as a reference to an object in memory.

            //No garbage collection: Value types do not require garbage collection, because they
            //are stored in memory as a single unit and do not occupy memory space that can be reclaimed.

            //Can be passed by value or by reference: Value types can be passed to methods by value or
            //by reference. When passed by value, a copy of the value type is made and passed to the method.
            //When passed by reference, a reference to the original value type is passed to the method
